...
class ProjectWizard(models.TransientModel):
    _name = 'mision_espacial.project.wizard'
    _description = 'Wizard: Quick Project for Mission Object.'

    def _default_mission(self):
        return self.env['mision_espacial.mission'].browse(self._context.get('active_id'))
    def _default_name(self):
        return self.env['mision_espacial.mission'].browse(self._context.get('active_id')).name
    def _default_launch_date(self):
        return self.env['mision_espacial.mission'].browse(self._context.get('active_id')).launch_date
    def _default_return_date(self):
        return self.env['mision_espacial.mission'].browse(self._context.get('active_id')).return_date
